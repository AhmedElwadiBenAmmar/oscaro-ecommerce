<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- Lien vers le catalogue front --}}
                    <x-nav-link :href="route('catalogue.index')" :active="request()->routeIs('catalogue.*')">
                        {{ __('Produits') }}
                    </x-nav-link>

                    {{-- Lien vers la sélection véhicule (optionnel) --}}
                    <x-nav-link :href="route('vehicle.form')" :active="request()->routeIs('vehicle.*')">
                        {{ __('Sélection véhicule') }}
                    </x-nav-link>
                </div>
            </div>

            <!-- Search + User Dropdown (desktop) -->
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-6">
                {{-- Formulaire de recherche catalogue --}}
                <form action="{{ route('catalogue.index') }}" method="GET" class="flex items-center gap-2">
                    <input
                        type="text"
                        name="q"
                        value="{{ request('q') }}"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('vehicle.form')" :active="request()->routeIs('vehicle.*')">
                {{ __('Sélection véhicule') }}
            </x-responsive-nav-link>
